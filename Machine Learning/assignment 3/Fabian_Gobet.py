...
Original file is located at
    https://colab.research.google.com/drive/13LjBAwmFfGDyfS0aUlgp8eAcFRGc5jUZ
...
class ReLU(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        d_x = (relu_x > 0).astype(int)*grad_in
...
class SoftmaxCrossEntropy(Loss):
    def forward(self, inputs: np.ndarray, targets: np.ndarray) -> float:
        ...
        mean_crossentropy = -np.mean(np.sum(one_hot_t*np.log(softmax),axis=1))
...
def train_one_step(model: Layer, loss: Loss, learning_rate: float, input: np.ndarray, target: np.ndarray, weight_decay=0.0005) -> float:
    ...
    for key, grad in variable_gradients.items():
      weight_decay_term = weight_decay*2*model.var[key]
      model.var[key] = model.var[key] - learning_rate*(grad +weight_decay_term)
...
for epoch in range(200):
    ...
    accumulated_loss, count = 0,0

    for i in range(0, len(train_validation_set) - n_train, batch_size):
      img = train_validation_set.images[train_indices[i:i + batch_size]]
      lbl = train_validation_set.labels[train_indices[i:i + batch_size]]

      mini_loss = train_one_step(model, loss, learning_rate, img, lbl)
      accumulated_loss, count = accumulated_loss + mini_loss, count + 1

    current_loss_value = accumulated_loss/count
    ...
    if validation_accuracy > best_validation_accuracy:
      ...
      if epoch - best_epoch > 10:
        print(f"Patience ran out at epoch {epoch}. Best accuracy was {best_validation_accuracy:.2f}%, at epoch {best_epoch}.")
        break
    # End
