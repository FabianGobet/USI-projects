...
class Field:
    ...
    def exchange_startall(self):
        ...
        if domain.neighbour_south>=0:
            self.recvRequests.append(domain.comm.Irecv(self.bdryS, source=domain.neighbour_south))
            self._buffS = self.inner[:,0].copy()
            self.sendRequests.append(domain.comm.Isend(self._buffS, dest=domain.neighbour_south))

        if domain.neighbour_east>=0:
            self.recvRequests.append(domain.comm.Irecv(self.bdryE, source=domain.neighbour_east))
            self._buffE = self.inner[-1,:].copy()
            self.sendRequests.append(domain.comm.Isend(self._buffE, dest=domain.neighbour_east))

        if domain.neighbour_west>=0:
            self.recvRequests.append(domain.comm.Irecv(self.bdryW, source=domain.neighbour_west))
            self._buffW = self.inner[0,:].copy()
            self.sendRequests.append(domain.comm.Isend(self._buffW, dest=domain.neighbour_west))
